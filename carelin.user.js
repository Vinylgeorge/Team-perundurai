...
(function () {
  ...
  const TARGET_REQUESTER_ID = "A45NRN4A6TJ14";
  ...
  function getRequesterIdFromPage() {
    try {
      const pathnameMatch = location.pathname.match(/^\/projects\/([^/]+)\/tasks\/[^/]+/i);
      if (pathnameMatch && pathnameMatch[1]) return pathnameMatch[1];
    } catch (e) {}

    try {
      const requesterLink = document.querySelector('a[href*="/requesters/"]');
      if (requesterLink && requesterLink.href) {
        const hrefMatch = requesterLink.href.match(/\/requesters\/([A-Z0-9]+)/i);
    ...
    try {
      ...
      const textMatch = html.match(/requester(?:Id|ID)?["'\s:>]+([A-Z0-9]{5,})/i);
  ...
  function isTargetRequester() {
    const requesterId = getRequesterIdFromPage();
    return requesterId === TARGET_REQUESTER_ID;
  }
